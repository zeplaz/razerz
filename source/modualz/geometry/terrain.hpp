...
void set_emiter(emetteur* in_emettuer)
{
  global_emetteur = in_emettuer;
}
  ...
  void load_texturez(std::string& terrain_displacement,std::string& terrin_defuse)
  {
    ...
    //GLuint          gltexture_displacement_glID;
    //GLuint          ;
  //  glEnable(GL_CULL_FACE); (\> no way texutr one? fun set to proper unit)
  ...
  /*
<template class func>
  void set_depth_callback((func in_func))
  {
    terrain_depth_callback =&in_func;
  }*/

  void change_depth_dis(float dmap_depth)
  {
      displacemap_depth  += dmap_depth;
  }
